...
def compute_avg_monthly_difference(time_series, first_year, last_year):
    ...
    if first_year == None  or first_year == "" or int(first_year) <= 0:
        #raise ExamException('Negative or zero value provided')
        first_year == '1949'
    ...
    if last_year == None or last_year == "" or int (last_year) <= 0:
        #raise ExamException('Negative or zero value provided')
        last_year == '1951'
    
    if int(last_year) < 1950 or int(last_year) > 1960 or last_year == "":
        raise ExamException('last_year must be between 1950 and 1960')

    if int(last_year) <= int(first_year) or int(first_year) >= int(last_year):
    ...
    for i in range(0, 12):
        ...
        while True:
            ...
            s = round( ( (s1 + s2) / (diff - 1) ), 2 )

            if ( int(y1[i]) == 0 or int(y2[i]) == 0 or int(y3[i]) == 0 or int(y4[i]) == 0 or int(y5[i]) == 0 or int(y6[i]) == 0 or int(y7[i]) == 0 or int(y8[i]) == 0 or int(y9[i]) == 0 or int(y10[i]) == 0 or int(y11[i]) == 0 or int(y12[i]) == 0 ):
                i = i + 1
                s = 0
            contatore += 1
            if contatore > int(last_year):
                break        
